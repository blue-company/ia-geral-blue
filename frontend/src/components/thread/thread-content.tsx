...
// Define o conjunto de tags XML que devem ser ocultadas durante o streaming
...
// Função auxiliar para renderizar anexos
export function renderAttachments(attachments: string[], fileViewerHandler?: (filePath?: string) => void, sandboxId?: string, project?: Project) {
    ...
    // Pré-carrega anexos no cache se tivermos um sandboxId
    if (sandboxId) {
        // Verifica se podemos acessar o localStorage e se há uma sessão de autenticação válida antes de tentar pré-carregar
        ...
        try {
            ...
            // Captura silenciosa - localStorage pode estar indisponível em alguns contextos
        }

        // Só tenta pré-carregar se tivermos uma sessão válida
        if (hasValidSession && token) {
            // Usa setTimeout para fazer isso de forma assíncrona sem bloquear a renderização
            setTimeout(() => {
...
// Renderiza conteúdo Markdown preservando tags XML que devem ser exibidas como chamadas de ferramentas
export function renderMarkdownContent(
    ...
    // Se estiver no modo de depuração, exibe apenas o conteúdo bruto em uma tag pre
    if (debugMode) {
        return (
    ...
    // Se nenhuma tag XML for encontrada, retorna o conteúdo completo como markdown
    ...
    while ((match = xmlRegex.exec(content)) !== null) {
        // Adiciona texto antes da tag como markdown
        ...
        if (toolName === 'ask') {
            ...
            // Usar as funções importadas no topo do arquivo
                ...
                <div className="mx-auto max-w-3xl">
                    {displayMessages.length === 0 && !streamingTextContent && !streamingToolCall &&
                        ...
                        <div className="space-y-8">
                                        ...
                                        return (
                                            <div key={group.key} ref={groupIndex === groupedMessages.length - 1 ? latestMessageRef : null}>
                                                <div className="flex items-start gap-3">
                                                    <div className="flex-shrink-0 w-8 h-8 mt-2 rounded-md flex items-center justify-center overflow-hidden ml-auto mr-2">
                                                        <Image src="/agent_eye_buddy.png" alt="InventuAI" width={28} height={28} className="object-contain invert dark:invert-0 opacity-70" />
                                                    </div>
                            ...
                            {/* Para modo de reprodução - Mostra animação de chamada de ferramenta se ativa */}
                            {readOnly && currentToolCall && (
                                <div ref={latestMessageRef}>
                                    <div className="flex items-start gap-3">
                                        <div className="flex-shrink-0 w-8 h-8 mt-2 rounded-md flex items-center justify-center overflow-hidden bg-primary/10">
                                            <Image src="/agent_eye_buddy.png" alt="InventuAI" width={28} height={28} className="object-contain" />
                                        </div>
                                        <div className="flex-1 space-y-2">
                                            <div className="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium text-primary bg-primary/10 rounded-md border border-primary/20">
                                                ...
                                                <span className="font-mono text-xs text-primary">
                                                    {currentToolCall.name || 'Usando Ferramenta'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {/* Para modo de reprodução - Mostra indicador de streaming se ainda não houver mensagens */}
