...
DECLARE
  ...
  user_name TEXT;
BEGIN
  -- Verificar se a conta j├í existe
  IF EXISTS (SELECT 1 FROM basejump.accounts WHERE id = user_id) THEN
    RAISE NOTICE 'Conta j├í existe para o usu├írio %', user_id;
    RETURN;
  END IF;

  -- Obter o email do usu├írio
  SELECT email INTO user_email FROM auth.users WHERE id = user_id;
  
  -- Extrair nome do usu├írio do email
  ...
  -- Inserir diretamente na tabela accounts com todos os campos necess├írios
  ...
  BEGIN
    ...
    RAISE NOTICE 'Conta criada com sucesso para o usu├írio %', user_id;
  ...
  -- Tamb├®m criar entrada na tabela account_user para permiss├Áes
  BEGIN
    -- Desativar temporariamente os triggers
    ...
    RAISE NOTICE 'Permiss├Áes configuradas para o usu├írio %', user_id;
  EXCEPTION WHEN OTHERS THEN
    -- Garantir que os triggers sejam reativados mesmo em caso de erro
    ALTER TABLE basejump.account_user ENABLE TRIGGER ALL;
    RAISE EXCEPTION 'Erro ao configurar permiss├Áes: %', SQLERRM;
  END;
END;
$$;
...
BEGIN
  ...
  -- Confirmar o usu├írio automaticamente (sem necessidade de verifica├º├úo por email)
  BEGIN
  ...
  -- 1. Inserir na tabela public.accounts (necess├íria para a chave estrangeira)
  INSERT INTO public.accounts (id)
  VALUES (NEW.id)
  ON CONFLICT (id) DO NOTHING;
  
  -- 2. Tentar inserir na tabela basejump.accounts para manter consist├¬ncia
  BEGIN
    INSERT INTO basejump.accounts (
      id,
      created_at,
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Ignorar erros na inser├º├úo em basejump.accounts
    -- O importante ├® que a conta exista em public.accounts
    RAISE NOTICE 'Error creating account in basejump schema: %', SQLERRM;
  END;
  
  -- 4. Inicializar o uso de prompts para o novo usu├írio
  BEGIN
  ...
  -- 5. Processar o c├│digo de convite, se existir
  BEGIN
    -- Verificar se o usu├írio foi registrado com um c├│digo de convite
    IF NEW.raw_app_meta_data IS NOT NULL AND 
       NEW.raw_app_meta_data::jsonb ? 'invite_code' THEN
      
      -- Extrair o c├│digo de convite (converter para UUID)
      DECLARE
        raw_invite_code TEXT;
      BEGIN
        raw_invite_code := NEW.raw_app_meta_data::jsonb->>'invite_code';
        
        -- Verificar se o formato ├® UUID
      ...
      IF invite_code IS NOT NULL THEN
        -- Verificar se o c├│digo de convite existe na tabela user_invites
        ...
        IF inviter_id IS NOT NULL THEN
          -- Marcar o convite como utilizado (mesmo que j├í tenha sido usado antes)
          ...
          -- Incrementar o contador de convites utilizados para o usu├írio que convidou
          UPDATE public.user_invites ui
          SET used_count = COALESCE(ui.used_count, 0) + 1
          WHERE ui.inviter_id = inviter_id
          AND ui.invite_code::TEXT = invite_code::TEXT;
          
          -- Adicionar um prompt extra ao limite di├írio do usu├írio que criou o convite
          -- Garantir que o usu├írio receba o b├┤nus para todas as datas futuras
          DECLARE
          ...
          BEGIN
            ...
            -- Calcular o n├║mero total de convites utilizados para este usu├írio
            SELECT COUNT(*) INTO bonus_prompts
            FROM public.user_invites
            WHERE inviter_id = inviter_id
            AND used = TRUE;
            
            -- Garantir que o bonus_count seja pelo menos igual ao n├║mero de convites utilizados
            ...
            IF inviter_usage_record IS NULL THEN
              -- Criar novo registro com bonus_count igual ao n├║mero de convites utilizados
              INSERT INTO public.prompt_usage (user_id, date, count, bonus_count)
              VALUES (inviter_id, CURRENT_DATE, 0, GREATEST(1, bonus_prompts));
            ELSE
              -- Atualizar o bonus_count no registro existente para ser pelo menos igual ao n├║mero de convites utilizados
...
DECLARE
  ...
  account_slug TEXT;
BEGIN
  -- Adicionar log para depura├º├úo
  ...
  IF NOT EXISTS (SELECT 1 FROM basejump.accounts WHERE id = NEW.account_id) THEN
    -- Adicionar log para depura├º├úo
    RAISE LOG 'Conta n├úo existe, tentando criar para account_id: %', NEW.account_id;
    
    -- Tentar obter o email do usu├írio
    BEGIN
      ...
      END IF;
      RAISE LOG 'Nome de usu├írio gerado: %', user_name;
    ...
    END;
  ELSE
    RAISE LOG 'Conta j├í existe para account_id: %', NEW.account_id;
  END IF;
...
CREATE OR REPLACE FUNCTION "public"."ensure_thread_account_id"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- Se o account_id n├úo foi especificado, use o account_id do projeto
  ...
  -- Se ainda n├úo tiver account_id, use o ID do usu├írio atual
...
DECLARE
  ...
  bonus_count_val INT := 0;
  total_allowed INT;
  today_invites INT := 0; -- Vari├ível para armazenar convites do dia
BEGIN
  -- Garantir que exista um registro para hoje
  PERFORM ensure_prompt_usage_record(p_user_id);
  
  -- Obter a contagem atual e o b├┤nus
  ...
  -- Verificar tamb├®m convites v├ílidos realizados no dia que ainda n├úo foram contabilizados
  
  -- Contar convites v├ílidos realizados no dia
  ...
  -- Adicionar os convites do dia ao bonus_count se n├úo estiverem j├í contabilizados
    ...
    BEGIN
      ...
      BEGIN
        IF invite_code_val IS NOT NULL THEN
          ...
          -- Se encontrou um convite v├ílido, incrementar o bonus_count do usu├írio que convidou
          IF inviter_id_val IS NOT NULL THEN
            -- Verificar se j├í existe um registro para o dia atual
            UPDATE prompt_usage
            SET bonus_count = COALESCE(bonus_count, 0) + 1
            WHERE user_id = inviter_id_val AND date = CURRENT_DATE;
            
            -- Se n├úo existir, criar um novo registro
            IF NOT FOUND THEN
      ...
      END;
    EXCEPTION WHEN OTHERS THEN
      -- Se n├úo for um UUID v├ílido ou n├úo existir, continuar sem erro
      NULL;
    END;
    
    -- Tamb├®m atualizar os convites para este e-mail
    UPDATE user_invites ui
    SET 
      used = TRUE, 
      used_at = NOW(), 
...
EXCEPTION
  WHEN OTHERS THEN
    -- Logar o erro mas permitir que a confirma├º├úo prossiga
    RAISE LOG 'Erro ao processar confirma├º├úo de convite para %: %', NEW.email, SQLERRM;
    RETURN NEW;
END;
$$;
...
BEGIN
  ...
  -- Se tiver um c├│digo de convite v├ílido, processar
  IF invite_code_val IS NOT NULL THEN
    -- Atualizar o convite com as informa├º├Áes do novo usu├írio
    UPDATE user_invites ui
    SET 
      used = TRUE, 
      used_at = NOW(), 
  ...
  -- Tamb├®m verificar se o e-mail do novo usu├írio corresponde a algum convite
  UPDATE user_invites ui
  SET 
    used = TRUE, 
    used_at = NOW(), 
...
EXCEPTION
  WHEN OTHERS THEN
    -- Logar o erro mas permitir que o usu├írio seja criado
...
BEGIN
  ...
  -- Verificar se o usu├írio j├í atingiu o limite
...
BEGIN
  -- Primeiro, confirmar todos os usu├írios que ainda n├úo foram confirmados
  UPDATE auth.users
  SET email_confirmed_at = NOW()
  ...
  -- Depois, inicializar os registros de prompt_usage para todos os usu├írios
  FOR user_record IN SELECT id FROM auth.users
  LOOP
    -- Chamar a fun├º├úo ensure_prompt_usage_record para cada usu├írio
    BEGIN
      PERFORM public.ensure_prompt_usage_record(user_record.id);
      processed_count := processed_count + 1;
      
      -- Verificar se o usu├írio tem convites utilizados
...
CREATE POLICY "Service role can manage all prompt_usage" ON "public"."prompt_usage" USING ((("auth"."jwt"() ->> 'role'::"text") = 'service_role'::"text"));
...
CREATE POLICY "Users can insert messages in their threads" ON "public"."messages" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."threads"
  WHERE (("threads"."thread_id" = "messages"."thread_id") AND ("threads"."account_id" = "auth"."uid"())))));
...
CREATE POLICY "thread_select_policy" ON "public"."threads" FOR SELECT USING ((("basejump"."has_role_on_account"("account_id") = true) OR (EXISTS ( SELECT 1
   FROM "public"."projects"
...
-- Script para corrigir automaticamente todas as contas
-- Este script faz três coisas:
-- 1. Cria o trigger necessário para criação automática de contas
-- 2. Cria contas para todos os usuários existentes que não têm contas
-- 3. Inicializa os registros de uso de prompts para todos os usuários

-- Parte 1: Criar a função e o trigger para criação automática de contas
...
-- Parte 2: Criar contas para todos os usuários existentes
DO $$
DECLARE
  user_record RECORD;
  user_email TEXT;
  ...
  basejump_account_exists BOOLEAN;
BEGIN
  -- Desativar temporariamente todos os triggers na tabela basejump.accounts
  SET session_replication_role = 'replica';
  
  FOR user_record IN SELECT * FROM auth.users
  LOOP
    -- Verificar se o usuário já tem uma conta em public.accounts
    SELECT EXISTS (
      SELECT 1 FROM public.accounts WHERE id = user_record.id
    ) INTO account_exists;
    
    -- Verificar se o usuário já tem uma conta em basejump.accounts
    SELECT EXISTS (
      SELECT 1 FROM basejump.accounts WHERE id = user_record.id
    ) INTO basejump_account_exists;
    
    -- Obter o email e nome do usuário
    user_email := user_record.email;
    IF user_email IS NOT NULL THEN
      user_name := split_part(user_email, '@', 1);
    ELSE
      user_name := 'User ' || substr(user_record.id::text, 1, 8);
    END IF;
    
    -- 1. Criar conta em public.accounts se não existir
    IF NOT account_exists THEN
      INSERT INTO public.accounts (id)
      VALUES (user_record.id)
      ON CONFLICT (id) DO NOTHING;
      
      RAISE NOTICE 'Created account in public.accounts for user %', user_record.id;
    END IF;
    
    -- 2. Criar conta em basejump.accounts se não existir
    IF NOT basejump_account_exists THEN
      BEGIN
        INSERT INTO basejump.accounts (
          id,
          created_at,
        ...
        RAISE NOTICE 'Created account in basejump.accounts for user %', user_record.id;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Error creating account in basejump schema for user %: %', user_record.id, SQLERRM;
      END;
    END IF;
    
    -- 4. Inicializar o uso de prompts para o usuário
    BEGIN
      INSERT INTO public.prompt_usage (
        user_id,
        date,
      ...
      RAISE NOTICE 'Initialized prompt usage for user %', user_record.id;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Error initializing prompt usage for user %: %', user_record.id, SQLERRM;
    END;
  END LOOP;
  ...
  RAISE NOTICE 'Finished processing all users';
END;
$$;

-- Parte 3: Criar função para garantir que um registro de prompt_usage exista para um usuário
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    bonus_prompts := 0;
    RAISE NOTICE 'Error calculating bonus prompts: %', SQLERRM;
  END;
